...
#
# Было 90 — по аналогии с порогом удаления, без замера. Замер на живой базе
# показал, что 90 дней не отсекают почти ничего: из 7412 чатов за 90 дней
# активны 5746, за месяц 4252, за две недели 1850, за неделю 1055. То есть
# гидрация грузила 5856 чатов вместо примерно 2000.
#
# Опускать этот порог безопасно, в отличие от порога удаления, и вот почему:
# чат, не поднятый в память, при первом же упоминании от Avito дочитывается из
# базы в _process_chat (was_cached=False) — и запроса в Avito это НЕ стоит,
# потому что last_message_at берётся из сводки и короткий выход срабатывает.
# Та же работа, просто размазанная по опросу, а не упёршаяся вся в старт.
#
# Непрочитанные поднимаются независимо от этого числа (см.
# list_chats_for_hydration) — иначе они пропали бы с экрана "Непрочитанные",
# который читает только кэш.
CHAT_HYDRATION_DAYS = 14
